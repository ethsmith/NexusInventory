...
package org.ExtendedAlpha.Nexus.Groups;
...
public class GroupManager {
    ...
    private GroupManager(NexusInventory plugin) {
        this.plugin = plugin;
    }

    public static GroupManager getInstance(NexusInventory plugin) {
        if (instance == null) {
    ...
    public void loadGroupsToMemory() {
        ...
        FileConfiguration groupsConfig = ConfigManager.getInstance().getConfig(ConfigType.GROUPS).getConfig();
    ...
    private void setDefaultsFile(String group) {
        ...
        if (!fileTo.exists()) {
            File fileFrom = new File(plugin.getDefaultFilesDirectory() + File.separator + "simpleplayerdata.json");
